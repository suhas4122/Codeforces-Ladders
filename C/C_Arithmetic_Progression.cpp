...
int main() {
    ...
    if(n == 1){
        ...
        if((arr[0] + arr[1])%2 == 0){
            ll d = arr[1] - arr[0];
            cout << 3 << "\n";
            cout << min(arr[0] - d, arr[1] + d) << " " << (arr[0] + arr[1])/2 << " " << max(arr[0] - d, arr[1] + d);
        }else{
            ll d = arr[1] - arr[0];
            cout << 2 << "\n";
            cout << min(arr[0] - d, arr[1] + d) << " " << max(arr[0] - d, arr[1] + d);
        }
    }else{
        sort(arr, arr+n);
        vector<ll>diff;
        ...
        if(diff[0] == diff[n-2]){
            cout << 2 << "\n";
            cout << min(arr[0] - diff[0], arr[n-1] + diff[0]) << " " << max(arr[0] - diff[0], arr[n-1] + diff[0]);
        }else if(diff[0]*2 == diff[n-2] && diff[0] == diff[n-3]) {
            cout << 1 << "\n";
            for1(n-1){
                ll x = arr[i] - arr[i-1];
                if(x == diff[n-2])
